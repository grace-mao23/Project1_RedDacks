...
def fullsearch(category):
    articles = newsapi(session['countrycode'], category) #news API
    newarticles = newyorktimesapi(session['country'], category) #NYT API
    return render_template('results.html', category = category, country = session['country'].capitalize(),
                                           articles = articles, newarticles = newarticles)
